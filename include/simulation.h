...
typedef struct {
  ...
  double battery_remaining; // Batterie de simulation : nombre de mouvements
                            // restant avant la fin.
  // On considère qu'un appel à envStep consomme une unité de batterie.
...
typedef struct {
  ...
  int numUsers;
  int numObstacles;
  double width, height, depth; // Dimensions de la carte. Attention, depth est
                               // la vraie hauteur ! (axe Z vers le haut)
